...
class ASSET_OT_Add(Operator, ImportHelper):
    ...
    def add_pack(self, file_path, assets_path, preset_names, context):
        ...
        handle.close()
        utils.AddonReloadManagement.reloadAssetList()
        self.report({'INFO'}, "asset pack successully added")
    ...
    def add(self, context):
        for file in self.files:
            ...
            preset_names = os.listdir(assets_path)

            # add / import
            if os.path.splitext(file_name)[1] == ".blend":
                self.add_file(file_path, assets_path, preset_names, context)   #   add single preset file
            else:
                self.add_pack(file_path, assets_path, preset_names, context)   #   add preset pack
